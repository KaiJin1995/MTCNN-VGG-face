...
def draw_and_show(im, bboxes,points=None):
    ...
    if num_boxes != 0:
        if num_boxes > 1:
            cv2.imwrite('/home/kjin/caffe-master/examples/VGGNet/pycaffe-mtcnn-master/PaperPic/VideoOriginal/Pic' + str(ErrorPic) + '.jpg', im)
        ...
        if num_boxes > 1:
            cv2.imwrite('/home/kjin/caffe-master/examples/VGGNet/pycaffe-mtcnn-master/PaperPic/VideoOriginal/' + str(ErrorPic) + '.jpg',im)
            ErrorPic=ErrorPic+1
                # if points.size:
                #     p = points[i]
                #     for i in range(5):
